...
let overrideCabal = drv: f: (drv.override (args: args // {
    ...
    extendHaskellPackages = haskellPackages: haskellPackages.override {
      overrides = self: super: {
        ...
        reflex-bug = self.callPackage ./bug {};
...
in rec {
  ...
  # nix-shell
  # cabal repl
  # 	Prelude> :set -ibug/src
  # 	Prelude> :l bug/src/Bug.hs
  #
  # nix-build -A reflex-bug
  # rm -rf result2; cp -rLf result result2
  reflex-bug = ghcjs.reflex-bug;
  ...
  mkSdist = pkg: pkg.override {
    mkDerivation = drv: ghc.mkDerivation (drv // {
      postConfigure = ''
        ./Setup sdist
        mkdir "$out"
        mv dist/*.tar.gz "$out/${drv.pname}-${drv.version}.tar.gz" # */ comment to appease confused syntax highlighting
        exit 0
      '';
    });
  };
